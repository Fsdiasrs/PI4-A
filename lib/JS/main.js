...
/*------------------------------------------------------------------------------
 *  header menu
 *  Função responsável por abrir e fechar o headermenu
 *------------------------------------------------------------------------------*/
function headerMenu(){
    ...
    document.querySelectorAll(".js-header-menu-toggler").forEach((item) => {
        item.addEventListener("click", toggleMenu); /* revisar 25:37  */
    });

    /* feche o menu clicando fora dele */
    ...
    menu.addEventListener("click", (event) => {
        ...
        window.innerWidth <= menuCollapseBreakpoint){
            /* evita o comportamento de clique de âncora padrão */
            event.preventDefault();

            /* se o item de menu já estiver expandido, reduza-o e saia */
             if(target.parentElement.classList.contains("active")){
                collapse();
                return;
             }
             /* se ainda não estiver expandido ... execute código abaixo*/

            /* recolher o outro item de menu expandido se existir */
